...
import { MonthlyInfo } from "../../../stores/monthly/model"
...
const EditQualityServiceModal: React.FC<Props> = props => {
  const {className,
    id,
    title,
    message,
  ...
  useEffect (() => {
    if(data != undefined && !Utilities.isEmptyObject(data)) {
      setNewWater(data.new_water_quantity)
      setNewElectric(data.new_electric_quantity)
      setSelectedMonthYear(DateUtils.toDate(data.month_year, "MM-yyyy"))
    }
...
export default EditQualityServiceModal
